...
create-db-migration: ## Create a new migration
	@alembic -c src/to_do_list_hexagonal/adapters/outbound/db/alembic/alembic.ini revision --autogenerate -m "$(m)"

apply-db-migration: ## Apply the migration
	@alembic -c src/to_do_list_hexagonal/adapters/outbound/db/alembic/alembic.ini upgrade head